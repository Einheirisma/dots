...
// 🆕 Метрика: количество уведомлений по статусу и каналу
var (
...
func main() {
	...
	// Удаление существующей очереди перед созданием
	_, _ = ch.QueueDelete("email_queue", false, false, false)
	...
	for d := range msgs {
		...
		if err := json.Unmarshal(d.Body, &msg); err != nil {
			...
			notificationStatusTotal.WithLabelValues("failed", "email").Inc() // 🔥 Инкрементируем метрику
			d.Nack(false, false)
			continue
		}
		...
		notificationStatusTotal.WithLabelValues("sent", "email").Inc() // 🔥 Инкрементируем метрику
		d.Ack(false)
	}
}
